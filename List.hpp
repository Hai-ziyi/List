/* List.hpp -- Industrial-grade dynamic array container (C++14 Optimized)
 * Requires: C++14 or later
 * Features: std::allocator support, reverse iterators, self-checking invariants,
 *           strong exception guarantee, EBO, optimized insert/emplace paths,
 *           built-in algorithms: sort, find, binary_search, rotate, slice, etc.
 *
...
public:
    ...
    reference       operator[](size_type i) noexcept       { LIST_DEBUG_ASSERT(i < sz_, "List::operator[] out of range"); return data_[i]; }
    const_reference operator[](size_type i) const noexcept { LIST_DEBUG_ASSERT(i < sz_, "List::operator[] const out of range"); return data_[i]; }
    ...
    // STL 风格：返回擦除数量
    size_type erase_val(const T& val) { return remove(val) ? 1 : 0; }
    ...
    // STL 风格：返回擦除数量
    size_type erase_all(const T& val) {
        size_type old = sz_;
        remove_all(val);
        return old - sz_;
    }
    ...
    // STL 风格：返回擦除数量
    template <typename Pred>
    size_type erase_if(Pred pred) { return remove_if(pred) ? 1 : 0; }
    ...
    // STL 风格：返回擦除数量
    template <typename Pred>
    size_type erase_all_if(Pred pred) {
        size_type old = sz_;
        remove_all_if(pred);
        return old - sz_;
    }
    ...
    void iota(T start) { for (size_type i = 0; i < sz_; ++i) data_[i] = start++; }
