/* List_pro.hpp -- High-performance dynamic array container (C++17/20 Enhanced)
 * Requires: C++17 or later (C++20 features enabled conditionally)
 * Features: if constexpr, C++17 algorithms, [[nodiscard]],
 *           spaceship<=> (C++20), likely/unlikely hints (C++20).
 *
 * Debug: define LIST_DEBUG_BOUNDS for operator[] range check.
 */
#ifndef LIST_PRO_HPP
#define LIST_PRO_HPP
...
#  define LIST_PRO_CPP20
#endif

/* ========== Debug assertion macros ========== */
#ifndef NDEBUG
#  define LIST_DEBUG_ASSERT(cond, msg) assert((cond) && msg)
#else
#  define LIST_DEBUG_ASSERT(cond, msg) ((void)0)
#endif

#ifdef LIST_PRO_CPP20
#  include <compare>
#endif

/* ========== Attribute macros ========== */
#ifdef LIST_PRO_CPP20
#  define LIST_NODISCARD [[nodiscard]]
#  define LIST_LIKELY    [[likely]]
#  define LIST_UNLIKELY  [[unlikely]]
#else
#  define LIST_NODISCARD
#  define LIST_LIKELY
#  define LIST_UNLIKELY
#endif
...
class List : private list_pro_detail::AllocBase<Alloc> {
    using alloc_base   = list_pro_detail::AllocBase<Alloc>;
...
public:
    ...
    List(std::initializer_list<T> il, const Alloc& a = Alloc())
        : alloc_base(a), data_(nullptr), sz_(0), cap_(0) {
        if LIST_UNLIKELY(il.size() == 0) return;
        reserve(il.size());
        for (auto& x : il) alloc_traits::construct(alloc(), &data_[sz_++], std::move(x));
        LIST_ASSERT_INV();
    }
    ...
    List(const List& o)
        ...
        if LIST_UNLIKELY(o.sz_ == 0) return;
        reserve(o.sz_);
        if constexpr (is_trivially_relocatable) {
            std::memcpy(data_, o.data_, o.sz_ * sizeof(T));
        } else {
            for (size_type i = 0; i < o.sz_; ++i)
                alloc_traits::construct(alloc(), &data_[i], o.data_[i]);
        }
        sz_ = o.sz_;
        LIST_ASSERT_INV();
    }
    ...
    /* ========== Allocator access ========== */
    LIST_NODISCARD allocator_type get_allocator() const noexcept { return alloc(); }

    /* ========== Element access ========== */
    LIST_NODISCARD reference at(size_type i) {
        if LIST_UNLIKELY(i >= sz_) throw std::out_of_range("List::at");
        return data_[i];
    }
    LIST_NODISCARD const_reference at(size_type i) const {
        if LIST_UNLIKELY(i >= sz_) throw std::out_of_range("List::at");
        return data_[i];
    }
    LIST_NODISCARD reference       operator[](size_type i) noexcept       { LIST_DEBUG_ASSERT(i < sz_, "List::operator[] out of range"); return data_[i]; }
    LIST_NODISCARD const_reference operator[](size_type i) const noexcept { LIST_DEBUG_ASSERT(i < sz_, "List::operator[] const out of range"); return data_[i]; }

    LIST_NODISCARD reference front() {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::front");
        return data_[0];
    }
    LIST_NODISCARD const_reference front() const {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::front");
        return data_[0];
    }
    LIST_NODISCARD reference back() {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::back");
        return data_[sz_ - 1];
    }
    LIST_NODISCARD const_reference back() const {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::back");
        return data_[sz_ - 1];
    }

    LIST_NODISCARD pointer       data() noexcept       { return data_; }
    LIST_NODISCARD const_pointer data() const noexcept { return data_; }

    /* ========== Iterators ========== */
    LIST_NODISCARD iterator               begin() noexcept        { return iterator(data_); }
    LIST_NODISCARD const_iterator         begin() const noexcept  { return const_iterator(data_); }
    LIST_NODISCARD const_iterator         cbegin() const noexcept { return const_iterator(data_); }
    LIST_NODISCARD iterator               end() noexcept          { return iterator(data_ + sz_); }
    LIST_NODISCARD const_iterator         end() const noexcept    { return const_iterator(data_ + sz_); }
    LIST_NODISCARD const_iterator         cend() const noexcept   { return const_iterator(data_ + sz_); }
    LIST_NODISCARD reverse_iterator       rbegin() noexcept       { return reverse_iterator(end()); }
    LIST_NODISCARD const_reverse_iterator rbegin() const noexcept { return const_reverse_iterator(end()); }
    LIST_NODISCARD const_reverse_iterator crbegin() const noexcept{ return const_reverse_iterator(end()); }
    LIST_NODISCARD reverse_iterator       rend() noexcept         { return reverse_iterator(begin()); }
    LIST_NODISCARD const_reverse_iterator rend() const noexcept   { return const_reverse_iterator(begin()); }
    LIST_NODISCARD const_reverse_iterator crend() const noexcept  { return const_reverse_iterator(begin()); }

    /* ========== Capacity ========== */
    LIST_NODISCARD bool      empty() const noexcept    { return sz_ == 0; }
    LIST_NODISCARD size_type size() const noexcept     { return sz_; }
    LIST_NODISCARD size_type capacity() const noexcept { return cap_; }

    LIST_NODISCARD size_type max_size() const noexcept {
        return std::min(alloc_traits::max_size(alloc()),
                        static_cast<size_type>(std::numeric_limits<difference_type>::max()));
    }

    void reserve(size_type new_cap) {
        if LIST_LIKELY(new_cap <= cap_) return;
        auto new_data = raw_alloc(new_cap);
        relocate(new_data, new_cap);
    }

    void shrink_to_fit() {
        if LIST_UNLIKELY(sz_ == cap_) return;
        if LIST_UNLIKELY(sz_ == 0) { raw_dealloc(data_, cap_); data_ = nullptr; cap_ = 0; return; }
        auto new_data = raw_alloc(sz_);
        relocate(new_data, sz_);
    }
    ...
    reference emplace_back(Args&&... args) {
        if LIST_UNLIKELY(sz_ >= cap_) grow();
        T* p = &data_[sz_];
    ...
    iterator emplace(const_iterator pos, Args&&... args) {
        ...
        if LIST_UNLIKELY(idx > sz_) throw std::out_of_range("List::emplace");
        if LIST_UNLIKELY(sz_ >= cap_) {
        ...
        try {
            ...
            if (idx < sz_) undo_shift_right(idx, 1);
            throw;
        }
        ++sz_;
        LIST_ASSERT_INV();
    ...
    iterator insert(const_iterator pos, size_type n, const T& val) {
        if LIST_UNLIKELY(n == 0) return iterator(data_ + (pos - cbegin()));
        size_type idx = static_cast<size_type>(pos - cbegin());
        if LIST_UNLIKELY(idx > sz_) throw std::out_of_range("List::insert");
        auto needed = sz_ + n;
        if LIST_UNLIKELY(needed > cap_) {
    ...
    void pop_back() {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::pop_back");
    ...
    void pop_front() {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::pop_front");
        erase(cbegin());
    }

    iterator erase(const_iterator pos) {
        size_type idx = static_cast<size_type>(pos - cbegin());
        if LIST_UNLIKELY(idx >= sz_) throw std::out_of_range("List::erase");
        erase_at(idx);
        --sz_;
        LIST_ASSERT_INV();
        return iterator(data_ + idx);
    ...
    iterator erase(const_iterator first, const_iterator last) {
        ...
        if LIST_UNLIKELY(beg > end || end > sz_) throw std::out_of_range("List::erase");
        size_type n = end - beg;
        if LIST_UNLIKELY(n == 0) return iterator(data_ + beg);
        erase_range(beg, n);
        sz_ -= n;
        LIST_ASSERT_INV();
    ...
    LIST_NODISCARD bool remove(const T& val) {
    ...
    // STL 风格：返回擦除数量
    LIST_NODISCARD size_type erase_val(const T& val) { return remove(val) ? 1 : 0; }
    ...
    // STL 风格：返回擦除数量
    LIST_NODISCARD size_type erase_all(const T& val) {
        size_type old = sz_;
        remove_all(val);
        return old - sz_;
    }

    LIST_NODISCARD T pop(size_type index = npos) {
        if (sz_ == 0) throw std::out_of_range("List::pop");
        if (index == npos) index = sz_ - 1;
        if LIST_UNLIKELY(index >= sz_) throw std::out_of_range("List::pop");
    ...
    LIST_NODISCARD bool is_sorted(Order order = Order::Ascending) const noexcept {
        if LIST_LIKELY(sz_ <= 1) return true;
        if (order == Order::Ascending) {
            for (size_type i = 1; i < sz_; ++i)
                if LIST_UNLIKELY(data_[i] < data_[i - 1]) return false;
        } else {
            for (size_type i = 1; i < sz_; ++i)
                if LIST_UNLIKELY(data_[i] > data_[i - 1]) return false;
        }
    ...
    template <typename Compare>
    LIST_NODISCARD bool is_sorted(Compare comp) const noexcept {
        if LIST_LIKELY(sz_ <= 1) return true;
        for (size_type i = 1; i < sz_; ++i)
            if LIST_UNLIKELY(comp(data_[i], data_[i - 1])) return false;
        return true;
    }
    ...
    void unique() {
        ...
        if constexpr (!std::is_trivially_destructible<T>::value) {
            std::destroy(data_ + j + 1, data_ + sz_);
        }
        sz_ = j + 1;
        LIST_ASSERT_INV();
    }
    ...
    LIST_NODISCARD difference_type find(const T& val) const noexcept {
        for (size_type i = 0; i < sz_; ++i)
    ...
    LIST_NODISCARD difference_type find_if(Pred pred) const {
        for (size_type i = 0; i < sz_; ++i)
            if (pred(data_[i])) return static_cast<difference_type>(i);
        return static_cast<difference_type>(-1);
    }

    LIST_NODISCARD difference_type rfind(const T& val) const noexcept {
    ...
    template <typename Pred>
    LIST_NODISCARD difference_type rfind_if(Pred pred) const {
        for (size_type i = sz_; i-- > 0;)
            if (pred(data_[i])) return static_cast<difference_type>(i);
        return static_cast<difference_type>(-1);
    }

    LIST_NODISCARD size_type index(const T& val) const {
        for (size_type i = 0; i < sz_; ++i)
            if (data_[i] == val) return i;
        throw std::out_of_range("List::index");
    }

    LIST_NODISCARD size_type count(const T& val) const noexcept {
        size_type c = 0;
        for (size_type i = 0; i < sz_; ++i)
            if (data_[i] == val) ++c;
        return c;
    ...
    template <typename Pred>
    LIST_NODISCARD size_type count_if(Pred pred) const {
        size_type c = 0;
        for (size_type i = 0; i < sz_; ++i)
            if (pred(data_[i])) ++c;
        return c;
    ...
    template <typename Pred>
    LIST_NODISCARD bool remove_if(Pred pred) {
        for (size_type i = 0; i < sz_; ++i) {
            if (pred(data_[i])) {
                erase(cbegin() + static_cast<difference_type>(i));
                return true;
    ...
    // STL 风格：返回擦除数量
    template <typename Pred>
    LIST_NODISCARD size_type erase_if(Pred pred) { return remove_if(pred) ? 1 : 0; }
    ...
    // STL 风格：返回擦除数量
    template <typename Pred>
    LIST_NODISCARD size_type erase_all_if(Pred pred) {
        size_type old = sz_;
        remove_all_if(pred);
        return old - sz_;
    }
    ...
    LIST_NODISCARD size_type lower_bound(const T& val) const {
        size_type l = 0, r = sz_;
        while (l < r) {
            size_type m = l + (r - l) / 2;
    ...
    LIST_NODISCARD size_type upper_bound(const T& val) const {
        size_type l = 0, r = sz_;
        while (l < r) {
            size_type m = l + (r - l) / 2;
            if (val < data_[m]) r = m;
    ...
    LIST_NODISCARD bool binary_search(const T& val) const {
        size_type i = lower_bound(val);
        return i < sz_ && !(val < data_[i]);
    }

    LIST_NODISCARD std::pair<size_type, size_type> equal_range(const T& val) const {
    ...
    template <typename Pred>
    LIST_NODISCARD bool all_of(Pred pred) const {
        for (size_type i = 0; i < sz_; ++i)
    ...
    template <typename Pred>
    LIST_NODISCARD bool any_of(Pred pred) const {
        for (size_type i = 0; i < sz_; ++i)
    ...
    template <typename Pred>
    LIST_NODISCARD bool none_of(Pred pred) const {
        for (size_type i = 0; i < sz_; ++i)
            if (pred(data_[i])) return false;
        return true;
    }

    LIST_NODISCARD T min() const {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::min");
        T res = data_[0];
        for (size_type i = 1; i < sz_; ++i)
    ...
    LIST_NODISCARD T max() const {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::max");
        T res = data_[0];
        for (size_type i = 1; i < sz_; ++i)
            if (data_[i] > res) res = data_[i];
        return res;
    ...
    // ── 统计方法（仅数值类型有意义，通用容器建议用 std::accumulate） ──
    // sum, product, average, median 已移除。通用容器不应内置数值运算。
    // 对数值型 List<T>，用 std::accumulate 替代。

    template <typename BinaryOp>
    LIST_NODISCARD T accumulate(T init, BinaryOp op) const {
        for (size_type i = 0; i < sz_; ++i)
            init = op(init, data_[i]);
        return init;
    }

    LIST_NODISCARD T accumulate(T init = T()) const {
        for (size_type i = 0; i < sz_; ++i)
            init += data_[i];
        return init;
    }

    LIST_NODISCARD List slice(size_type s, size_type e, size_type step = 1) const {
        if LIST_UNLIKELY(step == 0) throw std::invalid_argument("List::slice: step cannot be zero");
        if (s > sz_) s = sz_;
        if (e > sz_) e = sz_;
        List res;
        if LIST_UNLIKELY(s >= e) return res;
    ...
    LIST_NODISCARD List copy() const { return List(*this); }

    // starts_with / ends_with 已移除。对容器语义不清晰，建议用
    // std::equal(list.begin(), list.begin() + prefix.size(), prefix.begin())

    LIST_NODISCARD List operator+(const List& o) const {
        List res;
    ...
    LIST_NODISCARD bool operator==(const List& o) const noexcept {
        if (sz_ != o.sz_) return false;
        for (size_type i = 0; i < sz_; ++i)
            if (data_[i] != o.data_[i]) return false;
        return true;
    }
    LIST_NODISCARD bool operator!=(const List& o) const noexcept { return !(*this == o); }
    LIST_NODISCARD bool operator<(const List& o) const noexcept {
    ...
    LIST_NODISCARD bool operator>(const List& o) const noexcept { return o < *this; }
    LIST_NODISCARD bool operator<=(const List& o) const noexcept { return !(o < *this); }
    LIST_NODISCARD bool operator>=(const List& o) const noexcept { return !(*this < o); }

#ifdef LIST_PRO_CPP20
    auto operator<=>(const List& o) const {
        auto n = std::min(sz_, o.sz_);
        for (size_type i = 0; i < n; ++i) {
            if (auto c = data_[i] <=> o.data_[i]; c != 0) return c;
        }
        return sz_ <=> o.sz_;
    }
#endif
...
private:
    ...
    pointer raw_alloc(size_type n) {
        if LIST_UNLIKELY(n == 0) return nullptr;
    ...
    void destroy_all() noexcept {
        if constexpr (!std::is_trivially_destructible<T>::value) {
            std::destroy(data_, data_ + sz_);
        }
        sz_ = 0;
    }

    void destroy_tail(size_type keep) noexcept {
        if constexpr (!std::is_trivially_destructible<T>::value) {
            std::destroy(data_ + keep, data_ + sz_);
        }
    }

    /* ---- Move-or-copy helper for reallocation ---- */
    static auto pick_move(T& x) noexcept -> std::conditional_t<
    ...
    /* ---- Erase implementations (if constexpr) ---- */
    void erase_at(size_type idx) {
        if constexpr (is_trivially_relocatable) {
            std::memmove(data_ + idx, data_ + idx + 1, (sz_ - idx - 1) * sizeof(T));
        } else {
            std::destroy_at(std::addressof(*(data_ + idx)));
            std::move(data_ + idx + 1, data_ + sz_, data_ + idx);
            std::destroy_at(std::addressof(*(data_ + sz_ - 1)));
        }
    }

    void erase_range(size_type beg, size_type n) {
        if constexpr (is_trivially_relocatable) {
            std::memmove(data_ + beg, data_ + beg + n, (sz_ - beg - n) * sizeof(T));
        } else {
            std::move(data_ + beg + n, data_ + sz_, data_ + beg);
            std::destroy(data_ + sz_ - n, data_ + sz_);
        }
    }
    ...
    /* ---- Relocate ---- */
    void relocate(pointer new_data, size_type new_cap) {
        auto old_sz = sz_;
        if constexpr (is_trivially_relocatable) {
            if LIST_LIKELY(old_sz > 0) {
                std::memcpy(new_data, data_, old_sz * sizeof(T));
            }
        } else {
            if constexpr (is_nothrow_movable) {
                std::uninitialized_move(data_, data_ + old_sz, new_data);
            } else {
                size_type i = 0;
                try {
                    for (; i < old_sz; ++i)
            ...
            if constexpr (!std::is_trivially_destructible<T>::value) {
                std::destroy(data_, data_ + old_sz);
            }
        }
        raw_dealloc(data_, cap_);
    ...
    iterator insert_range(const_iterator pos, FwdIt first, FwdIt last,
        ...
        if LIST_UNLIKELY(n == 0) return iterator(data_ + idx);
        if LIST_UNLIKELY(idx > sz_) throw std::out_of_range("List::insert");
    ...
    LIST_NODISCARD size_type next_cap() const noexcept {
...
template <typename InputIt>
List(InputIt, InputIt) -> List<typename std::iterator_traits<InputIt>::value_type>;
template <typename T>
