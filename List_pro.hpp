...
public:
    ...
    LIST_NODISCARD reference       operator[](size_type i) noexcept       { return data_[i]; }
    LIST_NODISCARD const_reference operator[](size_type i) const noexcept { return data_[i]; }
    ...
    LIST_NODISCARD T sum() const {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::sum");
        T res = data_[0];
        for (size_type i = 1; i < sz_; ++i) res += data_[i];
        return res;
    }

    LIST_NODISCARD T product() const {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::product");
        T res = data_[0];
        for (size_type i = 1; i < sz_; ++i) res *= data_[i];
        return res;
    }

    LIST_NODISCARD double average() const {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::average");
        return static_cast<double>(sum()) / static_cast<double>(sz_);
    }

    LIST_NODISCARD T median() const {
        if LIST_UNLIKELY(empty()) throw std::out_of_range("List::median");
        List tmp(*this);
        tmp.sort();
        return tmp[sz_ / 2];  // lower middle for even-size lists
    }
    ...
    LIST_NODISCARD bool starts_with(const List& o) const {
        if LIST_UNLIKELY(o.sz_ > sz_) return false;
        for (size_type i = 0; i < o.sz_; ++i)
            if (data_[i] != o.data_[i]) return false;
        return true;
    }

    LIST_NODISCARD bool ends_with(const List& o) const {
        if LIST_UNLIKELY(o.sz_ > sz_) return false;
        for (size_type i = 0; i < o.sz_; ++i)
